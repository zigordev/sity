...
for (const road of roadNetwork.roads.values()) {
  ...
  if (road.spec.class === 'highway' || road.spec.class === 'ramp') {
    registerCorridor(
      road.samples.map((sample) => ({ x: sample.x, z: sample.z })),
      road.halfWidth + 3,
      `pavement:${road.spec.id}`,
      Boolean(road.spec.closed)
    );
  }
...
function registerRoadCuts(road: BuiltRoad) {
  ...
  for (const sample of samples) {
    const nearPortal = portalStations.some(
      (station) => Math.abs(sample.s - station) <= TUNNEL_APPROACH_CUT_M
    );
    const cutsTerrain =
      sample.structure === 'ground' || (sample.structure === 'tunnel' && nearPortal);
    ...
    registerCut(
      ...
      roadY,
      edge + 2.5,
      cutsTerrain && sample.y < sample.terrainY - 0.25,
      road.spec.id
    );
    if (edge < 4) {
      continue;
    ...
    for (const sign of [-1, 1]) {
      ...
      registerCut(
        ...
        roadY,
        1.8,
        cutsTerrain && sample.y < groundSurfaceBaseYAt(x, z) - 0.25,
        road.spec.id
      );
    }
  }
...
for (const junction of roadNetwork.junctions.values()) {
  ...
  registerCorridor(
    [junction.center, { x: junction.center.x + 0.01, z: junction.center.z }],
    radius + 4,
    `junction:${junction.nodeId}`
  );
  for (const corner of junction.corners) {
    const width = Math.max(
      ...[corner.fromRoadId, corner.toRoadId].map(
        (roadId) => roadNetwork.roads.get(roadId)?.cls.sidewalkWidth ?? 0
      )
    );
    registerCorridor(
      corner.points.map((point) => ({ x: point.x, z: point.z })),
      width + 0.6,
      `pavement:corner:${junction.nodeId}`
    );
  }
  ...
  if (destination) {
    ...
    const stripHalf = 3.2;
    for (
      let across = -half + stripHalf;
      across <= half - stripHalf + 0.01;
      across += stripHalf * 1.6
    ) {
      const clamped = Math.min(across, half - stripHalf);
      registerCorridor(
        [
          {
            x: origin.x + axis.x * -1 + destination.right.x * clamped,
            z: origin.z + axis.z * -1 + destination.right.z * clamped,
          },
          {
            x: origin.x + axis.x * (reach + 1) + destination.right.x * clamped,
            z: origin.z + axis.z * (reach + 1) + destination.right.z * clamped,
          },
        ],
        stripHalf + 0.6,
    ...
    for (let along = 0; along <= reach; along += 6) {
      for (let across = -half; across <= half; across += 6) {
        ...
        registerCut(
          ...
          roadY,
          3.5,
          origin.y < groundSurfaceBaseYAt(x, z) - 0.25,
          `destination:${junction.nodeId}`
        );
      }
    }
  }
