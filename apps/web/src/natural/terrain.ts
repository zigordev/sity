...
export function terrainMicroNoise(x: number, z: number) {
  ...
  const riverMask = THREE.MathUtils.smoothstep(
    riverDistance,
    RIVER_WIDTH_M * 1.1,
    RIVER_WIDTH_M * 3.4
  );
  ...
  const mountainMask =
    1 -
    THREE.MathUtils.smoothstep(Math.max(mountainHeightAt(x, z), snowMountainHeightAt(x, z)), 0, 36);
  const coastMask =
    1 - THREE.MathUtils.smoothstep(x, mainBoundaryMaxX - 420, mainBoundaryMaxX - 170);
  ...
  const drainage = Math.max(0, Math.sin((x - z) * 0.0068) * 0.5 + 0.5 - 0.52);
  const relief =
    (broad * 0.46 + middle * 0.34 + fine * 0.2 + drainage * 0.36) * MICRO_TERRAIN_HEIGHT_M;
...
export function addMicroDisplacedGrassTerrain(floor?: PavementFloor) {
  ...
  for (let row = 0; row <= rows; row += 1) {
    ...
    for (let column = 0; column <= columns; column += 1) {
      ...
      const height = Math.max(mountainHeightAt(x, z), snowMountainHeightAt(x, z));
      heights.push(height);
      const displacedY = Math.min(
        groundSurfaceYAt(x, z) + terrainMicroNoise(x, z),
        cutLimitAt(x, z)
      );
...
export function addMountainContourRidges(
  ...
  for (let ridgeIndex = 0; ridgeIndex < ridgeCount; ridgeIndex += 1) {
    ...
    for (let sample = 0; sample <= samples; sample += 1) {
      ...
      if (
        isInsideBounds(point, bounds, 4) &&
        height > minHeight &&
        pavementClearance(x, z, 30) > 4
      ) {
...
export function mountainHeightAt(x: number, z: number) {
  ...
  const ridgeNoise =
    0.93 + 0.05 * Math.sin(x * 0.021 + z * 0.013) + 0.035 * Math.sin(x * 0.009 - z * 0.017);
...
export function createMountainSurfaceGeometry(
  ...
  for (let zIndex = 0; zIndex <= MOUNTAIN_GRID_SEGMENTS; zIndex += 1) {
    ...
    const z = THREE.MathUtils.lerp(mountainVisibleBounds.minZ, mountainVisibleBounds.maxZ, zRatio);
...
export function snowMountainHeightAt(x: number, z: number) {
  ...
  const summitLift = 0.8 + 0.2 * Math.pow(Math.max(0, 1 - summitDistance / 0.76), 1.55);
  const ridgeLift = 1 + 0.055 * Math.cos(ridgeAngle * 2.1 + distance * 4.8) * centerMass;
...
export function setSnowMountainVertexColor(height: number, color: THREE.Color) {
  ...
  const snowBlend = THREE.MathUtils.smoothstep(
    (height - SNOW_MOUNTAIN_SNOWLINE_M) / (SNOW_MOUNTAIN_HEIGHT_M - SNOW_MOUNTAIN_SNOWLINE_M),
    0,
    1
  );
...
export function westernHillsHeightAt(x: number, z: number) {
  const mask =
    1 - THREE.MathUtils.smoothstep(x, WESTERN_HILLS_START_X_M - 380, WESTERN_HILLS_START_X_M);
  if (mask <= 0) {
    return 0;
  }
  ...
  const farm = THREE.MathUtils.smoothstep(
    z,
    WESTERN_COUNTRY_SPLIT_Z_M - 120,
    WESTERN_COUNTRY_SPLIT_Z_M + 120
  );
...
export function terrainSurfaceYAt(point: GroundPathPoint) {
  return (
    GRASS_SURFACE_Y + mountainHeightAt(point.x, point.z) + westernHillsHeightAt(point.x, point.z)
  );
}
...
export function fullTerrainSurfaceYAt(point: GroundPathPoint) {
  ...
  return Math.max(baseTerrainY, snowMountainSurfaceYAt(point.x, point.z, snowHeight));
}
